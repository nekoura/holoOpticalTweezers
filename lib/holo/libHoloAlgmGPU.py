...
class GSW(Holo):
    ...
    def iterate(self) -> tuple:
        ...
        for n in range(self.maxIterNum):
            ...
            # 向像平面光场添加权重(See Eq.19)
            self.Aw[self.signalRegion] = (
                cp.abs(self.Ak[self.signalRegion]) *
                (cp.abs(self.Atarget[self.signalRegion]) / cp.abs(self.AK[self.signalRegion])) ** 0.5
            )

            self.Ak = self.Aw * (self.AK / cp.abs(self.AK))
